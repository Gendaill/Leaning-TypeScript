...
function buildArray({ items, sortOrder }: { items: number; sortOrder: 'ascending' | 'descending'; }):number[] {
    let randomNumbers = [];
    let nextNumber;
...
let myArray1 = buildArray({ items: 12, sortOrder: 'ascending' });
let myArray2 = buildArray({ items: 8, sortOrder: 'descending' });
...
function loanCalculator ({ principle, interestRate, months = 12 }: { principle: number; interestRate: number; months?: number; }): string {
